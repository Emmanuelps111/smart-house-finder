...
BEGIN
  ...
  -- Gate: only proceed if the sender actually sent a connect request to me.
  -- Without this, any signed-in user could harvest other users' phone numbers.
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No pending roommate request from this user.';
  END IF;
  ...
  IF _prop_key IS NOT NULL AND length(_prop_key) > 0 THEN
    ...
    IF _my_req_id IS NOT NULL THEN
      UPDATE public.roommate_requests
        SET status = 'matched',
            match_partner_id = _sender_req_id,
            updated_at = now()
        WHERE id = _my_req_id;
    END IF;
